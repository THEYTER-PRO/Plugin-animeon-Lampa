(function () {
    ...
    function startPlugin() {
        ...
        if (!window.Lampa.Listener) return;

        window.Lampa.Listener.follow('full', function (e) {
            if (!e) return;
            if (e.type !== 'complite') return;

            var $ = window.jQuery || window.Lampa.jQuery;
            if (!$) return;
            ...
            if (!render) return;
            if (render.find('.button--animeon').length > 0) return;
    ...
    function searchAnime(query) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (results.length === 0) {
                    if (window.Lampa && window.Lampa.Noty) window.Lampa.Noty.show('Ничего не найдено');
                    return;
                }
            ...
            error: function () {
                if (window.Lampa && window.Lampa.Noty) window.Lampa.Noty.show('Ошибка сети');
            }
        });
    }
    ...
    function parsePlayerPage(pageUrl, title) {
        ...
        if (window.Lampa && window.Lampa.Noty) window.Lampa.Noty.show('Загрузка...');
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (streamUrl) {
                    ...
                    if (window.Lampa && window.Lampa.Noty) window.Lampa.Noty.show('Видео не найдено');
                }
            },
            error: function () {
                if (window.Lampa && window.Lampa.Noty) window.Lampa.Noty.show('Ошибка загрузки плеера');
            }
        });
    }
